...
If $r$ is an defective eigenvalue of the matrix $A$ with eigenvector $\vec{v}$, a \emph{\myindex{generalized eigenvector}} of $A$ is a vector $\vec{w}$ so that $(A - rI)\vec{w} = \vec{v}$. This is the same as the normal eigenvector equation with $\vec{v}$ on the right-hand side instead of $\vec{0}$. Since $(A - rI)\vec{v} = \vec{0}$, this also means that 
\[ 
    (A - rI)^2\vec{w} = 0. 
\] 
