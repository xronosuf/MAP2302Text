...
%% Validation Technique for the first two exercises needs to be written/determined.
\begin{exercise}
...
\begin{exercise}
    ...
    \begin{multipleChoice}
        \choice[correct]{$A$ is definitely not invertible.}
        \choice{$A$ might be invertible but more work is needed.}
        \choice{Nothing, because the Fredholm Alternative doesn't apply.}
        \choice{$A$ is definitely invertible}. 
    \end{multipleChoice}
\end{exercise}
%\comboSol{%
%}
    ...
    \begin{problem}
        ...
        \begin{problem}
            ...
            \begin{multipleChoice}
                ...
                \choice{There is (exactly) one solution, but we must do more work to determine what it is.}
                \choice{There is (exactly) one solution, and it is $\vec{x}=0$}
                \choice[correct]{There are an unknown number of non-zero solutions} 
...
\begin{exercise}%
    ...
    \begin{problem}
        What does this tell you about the invertibility of $A$? 
        \begin{multipleChoice}
            \choice{$A$ is definitely not invertible.}
            \choice{$A$ might be invertible but more work is needed.}
            \choice[correct]{$A$ is definitely invertible}. 
        \end{multipleChoice}
        \begin{problem}
            How about the solutions to $A\vec{x} = \vec{0}$? 
            \begin{multipleChoice}
                \choice{There are infinitely many solutions.}
                \choice[correct]{There is (exactly) one solution, but we must do more work to determine what it is.}
                \choice{There is (exactly) one solution, and it is $\vec{x}=0$}
                \choice{There are an unknown number of non-zero solutions} 
                \choice{There are no solutions}
            \end{multipleChoice}
        \end{problem}
    \end{problem}
...
%% Validator of the next several problems needs to be created.
\begin{exercise}
...
%% Validators of the next few problems need to be written.
\begin{exercise}
...
%%%% Most, if not all, of the problems below this point are included in other practice problem sections.

%\begin{exercise}%
%    Consider a set of 3 component vectors.
%    \begin{itemize}
%        \item How can it be shown if these vectors are linearly independent?
%        \item Can a set of 4 of these 3 component vectors be linearly independent? Explain your answer.
%        \item Can a set of 2 of these 3 component vectors be linearly independent? Explain.
%        \item How would it be shown if these vectors make up a spanning set for all 3 component vectors?
%        \item Can 4 vectors be a spanning set? Explain.
...
%    \begin{itemize}
%        \item Compute the rank of $A$ to determine how many of these vectors are linearly independent.
...
%    \begin{itemize}
%        \item Compute the trace and determinant of the matrix, and
%        \item Find the eigenvalues of the matrix and verify that the trace is the sum of the eigenvalues and the determinant is the product. 
%    \end{itemize}
%    \begin{equation*}
