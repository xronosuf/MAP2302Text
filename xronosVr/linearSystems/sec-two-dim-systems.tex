...
Since these are all based on the relation between $T$ and $D$, we can also combine all of this into a figure to summarize the details. In \figurevref{fig:TDPlaneAnalysis}, $T$ is on the horizontal axis and $D$ is the vertical axis. The graph drawn is $D = \frac{T^2}{4}$, which is the important criteria that shows up in the table. 
