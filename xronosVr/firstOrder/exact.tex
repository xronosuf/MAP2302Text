...
\end{mywrapfig}
We are interested in the lines of constant energy, that is lines where the energy is conserved;  we want curves where $F(x,y) = C$, for some constant $C$, since $F$ represents the energy of the system.  In our example, the curves $x^2+y^2=C$ are circles.  See \figurevref{exact:circlesfig}.
...
\begin{theorem}[thm:Poincare]{Poincar\'e}
...
\begin{exampleSol}
    ...
    \end{equation*}
    The implicit solution is $\operatorname{arctan} \bigl( \frac{y}{x} \bigr) = C$.  Solving, $y = \tan(C) x$.  That is, the solution is a straight line.  Solving $y(1) = 2$ gives us that $\tan(C) = 2$, and so $y= 2x$ is the desired solution. See \figurevref{exact:y2x}, and note that the solution only exists for $x > 0$.
    \begin{myfig}
        \capstart
